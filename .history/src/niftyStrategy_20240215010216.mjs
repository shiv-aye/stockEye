...
function mapStockData(stockData) {
    ...
    for (let i = 0; i < timestamps.length; i++) {
        // Check if closing price exists and is not empty
        if (closePrices[i] !== null && closePrices[i] !== undefined) {
            const date = new Date(timestamps[i] * 1000);

            // Exclude data falling within the specified time range
            if (!(date.getFullYear() === 2020 || (date.getFullYear() === 2021 && date.getMonth() === 0))) {
                ...
                mappedData.push({
                    date: date.toISOString().split('T')[0], // Convert timestamp to date
                    closingPrice,
                    percentChangeDay,
                    percentChangeWeek
                });
